...
'''#####################################################Only class used by train_PEBBLE.py#####################################################'''
class RewardModel:
    ...
    def uniform_sampling(self):
        ...
        print('sample shape = ' + str(sa_t_1.shape))
        print('label shape = ' + str(r_t_1.shape))
    ...
    def custom_clustering_sampling(self):
        ...
        # Add samples from replay buffer also
        #max_len = self.capacity if self.buffer_full else self.buffer_index

        #buf_sa_1 = self.buffer_seg1[:max_len]
        #buf_sa_2 = self.buffer_seg2[:max_len]

        #buf_comb = np.concatenate([buf_sa_1, buf_sa_2], axis=0)

        #total_comb = np.concatenate([sa_comb, buf_comb], axis=0)
        ...
        for i in range(attempt):
            ...
            #temp_dp = d_p.reshape(d_p.shape[0], d_p.shape[1]*d_p.shape[2])
            #temp_dp = d_p.reshape(-1, d_p.shape[-1])
            #temp_dp = d_p

            #print('shape of temp_dp: ' + str(temp_dp.shape))
            #print('length of d_c: ' + str(len(d_c)))
        ...
        #half_samples = self.mb_size // n_clusters
        # Should be mb_size for each sa_t not in total so double half_samples to fix?
        half_samples = self.mb_size

        #samp1, true_in1 = self.stratified(sa_comb, best_dc1, n_clusters, half_samples)
        ...
        for i in range(n_clusters):
            ...
            while len(cluster_samps) < half_samples:
                ...
                #samp1.extend(minmax[0])
                cluster_samps.extend(minmax[0])
                #true_in1.extend(minmax[1])
                true_in_samps.extend(minmax[1])

            samp1.append(cluster_samps)
            true_in1.append(true_in_samps)

        print('Len true_in = ' + str(len(true_in1[0])))
        # How to get correct index of r_t 1/2
        # Add to the d_c array so the class matches the index of the original data input
    ...
    def stratified(self, data, labels, n_c, samp):
        sel_samp = []
        sel_ind = []

        for cluster in range(n_c):
            cluster_ind = np.where(labels == cluster)[0]

            if len(cluster_ind) > samp:
                np.random.shuffle(cluster_ind)
                samp_ind = cluster_ind[:samp]
            else:
                samp_ind = cluster_ind

            sel_samp.extend(data[samp_ind])
            sel_ind.extend(samp_ind)
        


    def collect_clusters(self, data, clusters):
        ...
        try:
            while n_c <= max_nc:
                for i in range(len(comb)):
                    if n_c <= max_nc:
                        c = Cluster(comb[0], 2)
                        xx1, xx2 = c.get_clusters()
                        if len(xx1) == 0 or len(xx2) == 0:
                            comb.append(comb[0][0])
                            comb.pop(0)
                        else:
                            comb.pop(0)
                            comb.append(xx1)
            ...
            print('Retry...')
            print(e)
            return self.custom_clustering_sampling
...
class Cluster:
    ...
    def kmeans_new(self, start=0, max_iters=1000, tol=1e-5):
        if start == 0:
            sp = self.reg_start()
        elif start == 1:
            sp = self.start_plus()
        ...
        self.centroids = np.array(sp)

        for iteration in range(max_iters):
            distances = np.array([[self.calculate_distance(centroid, point) for centroid in self.centroids] for point in self.data])
            assignments = np.argmin(distances, axis=1)

            new_centroids = np.array([self.data[assignments==k].mean(axis=0) if np.any(assignments==k) else self.centroids[k] for k in range(self.n_clusters)])
