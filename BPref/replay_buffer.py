...
class ReplayBuffer(object):
    ...
    # Adds data into their respective arrays at the given index
    ...
    # Adds a batch of experience to the buffer
    # Calculates the next index after adding the batch
    # Handles if the buffer is full, wraps around
    # If buffer is not full, copies batch into buffer from idx to netx_index
    # If the buffer was full and wrapped idx is updated to remain
    ...
    # ##### Re-labeling of the buffer done here - potential for improvement #####
    # Updates rewards stored in buffer based on predictions made by reward predictor model
    # Iterates through the buffer, processes batches to be correct format
    # Uses the predictor to predict rewards based on the input
    # Update the rewards stored in the buffer
    ...
    # Generates random integers for sampling
    # Determines upper limit for sampling 
    # Convert selected data to pytorch tensors
    # Returns sampled batch consiting of pytorch tensors 
    ...
    # Similair to sample, but includes full observation
    # Randomly selects sample indices
    # Converts selected to tensors
    # Prepare full observation tensor
    # Returns the sampled batch of tensors
